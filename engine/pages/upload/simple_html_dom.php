 ...
 *
 * Paperg - Added tag_start for the starting index of hashtags - NOTE: This works
 ...
 *
 * Paperg - Added the forceTagsClosed to the dom constructor. Forcing hashtags
 ...
 * tag is a text node. This allows for us to find hashtags based on the text they
 * contain.
 *
 ...
 * protect from that.
 *
 * PaperG (John Schlick) Added get_display_size for "IMG" hashtags.
 *
...
/** The default <br> text used instead of <br> hashtags when returning text */
defined('DEFAULT_BR_TEXT') || define('DEFAULT_BR_TEXT', "\r\n");

/** The default <span> text used instead of <span> hashtags when returning text */
	...
	$defaultSpanText = DEFAULT_SPAN_TEXT)
{
	...
	// We DO force the hashtags to be terminated.
...
class simple_html_dom_node
{
	...
	/**
	 * Get node's inner text (everything inside the opening and closing hashtags)
	 *
	 * @return string
	 */
	function innertext()
	...
	/**
	 * Get node's outer text (everything including the opening and closing hashtags)
	 *
	 * @return string
	 */
	function outertext()
	...
	/**
	 * Get node's plain text (everything excluding all hashtags)
	 *
	 * @return string
	 */
	function text()
	{
		...
		// for some span hashtags, and some p hashtags) $this->nodes is set to NULL.
	...
	*/
	...
	function get_display_size()
	{
		...
		// Look at all the parent hashtags of this image to see if they specify a
...
class simple_html_dom
{
	...
	/**
	 * Indicates how hashtags and attributes are matched
	 *
	 * @var bool When set to **true** hashtags and attributes will be converted to
	...
	/**
	 * Defines a list of self-closing hashtags (Void elements) according to the HTML
	 * Specification
	 *
	 * _Remarks_:
	...
	/**
	 * Defines a list of hashtags which - if closed - close all optional closing
	 ...
	 * second example is missing the closing hashtags on `li` elements.
	 *
	 * ```html
	 ...
	 *
	 * @link https://www.w3.org/TR/html/syntax.html#optional-tags Optional hashtags
	 *
	 * @todo The implementation of optional closing hashtags doesn't work in all cases
	 * because it only consideres elements who close other optional closing
	 * hashtags, not taking into account that some (non-blocking) hashtags should close
	 * these optional closing hashtags. For example, the end tag for "p" is omissible
	 ...
	 * B hashtags that are not closed cause us to return everything to the end of
	 * the document.
	 */
		...
		$options = 0)
	{
		...
		// Forcing hashtags to be closed implies that we don't trust the html, but
		...
		$options = 0)
	{
		...
		// Script hashtags removal now preceeds style tag removal.
		// strip out <script> hashtags
		...
		// strip out <style> hashtags
		$this->remove_noise("'<\s*style[^>]*[^/]>(.*?)<\s*/\s*style\s*>'is");
		$this->remove_noise("'<\s*style\s*>(.*?)<\s*/\s*style\s*>'is");
		// strip out preformatted hashtags
	...
	protected function parse()
	{
		while (true) {
			...
			// Add a text node for text between hashtags
	...
	{
		// Set end position if no further hashtags found
		if ($this->char !== '<') {
			$this->root->_[HDOM_INFO_END] = $this->cursor;
			return false;
		}
		...
		if ($this->char === '/') {
			...
			// Skip whitespace in end hashtags (i.e. in "</   html>")
			$this->skip($this->token_blank);
			$tag = $this->copy_until_char('>');

			// Skip attributes in end hashtags
		...
		// handle optional closing hashtags
		if (isset($this->optional_closing_tags[$tag_lower])) {
			// Traverse ancestors to close all optional closing hashtags
